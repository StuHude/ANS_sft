#!/bin/bash
# Train for exactly 100 optimizer steps (8 GPUs) with bs=16, topk=512, save checkpoint_step_100.pth, then run 8-GPU RefCOCO eval.
...
RUN_NAME=pseudo_gumbel_v2_train100_8gpu_bs16_topk512
...
echo "===================================================================================="
echo "Train 100 steps then RefCOCO eval (8 GPU train + 8 GPU eval)"
echo "Run:        ${RUN_NAME}"
echo "Output:     ${OUT_DIR}"
...
nohup bash -lc '
  ...
  export PSEUDO_GUMBEL_DEBUG_GENERATE=1

  echo \"[INFO] Starting training (max_steps=${CKPT_STEP})...\"
  torchrun --nproc_per_node=8 --master_port=29740 \
  ...
  torchrun --nproc_per_node=8 --master_port=29741 \
