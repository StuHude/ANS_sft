#!/bin/bash
# Train for exactly 300 optimizer steps (8 GPUs), save checkpoint_step_300.pth, then run 8-GPU RefCOCO eval.
#
# Training uses the pseudo-gumbel loop + RefCOCO branch inside:
...
#
# Notes:
# - `train_pseudo_gumbel_v2.py` saves once at the exact max step and then stops.
# - Eval loads the HF model (`/data/xyc/ANS/pretrain_hf`) + optional base `.pth`, then applies the saved checkpoint.
...
RUN_NAME=pseudo_gumbel_v2_train300_8gpu_eosstat
OUT_DIR=/data/xyc/ANS/work_dirs/${RUN_NAME}
LOG_FILE=/data/xyc/ANS/work_dirs/${RUN_NAME}.train.log

CKPT_STEP=300
CKPT_PATH=${OUT_DIR}/checkpoint_step_${CKPT_STEP}.pth

EVAL_LOG=/data/xyc/ANS/work_dirs/${RUN_NAME}.refcoco_eval_8gpu.log
PIPELINE_LOG=/data/xyc/ANS/work_dirs/${RUN_NAME}.pipeline.log

echo "===================================================================================="
echo "Train 300 steps then RefCOCO eval (8 GPU train + 8 GPU eval)"
echo "Run:        ${RUN_NAME}"
echo "Output:     ${OUT_DIR}"
...
nohup bash -lc '
  ...
  torchrun --nproc_per_node=8 --master_port=29730 \
    projects/llava_sam2/mask_caption_sft/train_pseudo_gumbel_v2.py \
  ...
  echo \"[INFO] Training finished. Starting 8-GPU RefCOCO eval...\" | tee -a ${LOG_FILE}
