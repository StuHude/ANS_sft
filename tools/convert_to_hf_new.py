...
def main():
    ...
    model._merge_lora()
    # —— 合并 LoRA 之后，补齐假 PEFT 属性，避免 xtuner 的导出逻辑访问报错 ——
    lm = getattr(model.mllm.model, 'language_model', model.mllm.model)

    # 1) 没有要单独保存的模块
    setattr(lm, 'modules_to_save', None)

    # 2) 伪造一个最小可用的 peft_config，并指向 default 适配器
    class _DummyCfg:
        bias = 'none'
        inference_mode = False

    if not hasattr(lm, 'peft_config') or lm.peft_config is None:
        lm.peft_config = {'default': _DummyCfg()}
    else:
        lm.peft_config.setdefault('default', _DummyCfg())

    # 3) 指定当前激活的适配器名，避免 key 查找失败
    setattr(lm, 'active_adapter', 'default')

    #（如果你之前加过 lm.peft_config = {}，请删掉那行；要么像上面这样包含 'default'）
