...
# === Sa2VA minimal patch: ensure visual anchors ===
SPECIAL_IMG_TOKEN = "<IMG_CONTEXT>"
VP_L, VP_R = "<vp>", "</vp>"

def ensure_visual_anchors(text: str, is_video: bool) -> str:
    """在文本末尾追加视觉锚点，占位符与训练保持一致。"""
    s = (text or "").replace("<image>", SPECIAL_IMG_TOKEN)  # 关键映射！
    if SPECIAL_IMG_TOKEN not in s:
        if s and not s.endswith(" "):
            s += " "
        s += SPECIAL_IMG_TOKEN
    if is_video and (VP_L not in s and VP_R not in s):
        s = f"{VP_L} {s} {VP_R}"
    return s

def ensure_image_tag(text: str) -> str:
    s = text or ""
    return s if "<image>" in s else f"<image>\n{s}"

def is_video_dataset(name: str) -> bool:
    return (name or "").upper() in {"REVOS", "MEVIS", "MEVIS_U", "REFYTVOS", "DAVIS"}
# === End patch ===
...
if __name__ == '__main__':
    ...
    for item in tqdm.tqdm(dataloader):
        with torch.no_grad():
            text_prompt = ensure_image_tag(item['text_prompt'])
