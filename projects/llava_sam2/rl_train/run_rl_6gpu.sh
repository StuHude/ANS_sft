#!/bin/bash
#
# 6-GPU Distributed RL Training Script - FULL EPOCH
#
# This script uses torchrun for distributed training across 6 GPUs
...
echo "========================================================================"
echo "Sa2VA Dual-Loop RL Training - 6 GPU Full Epoch"
echo "========================================================================"
echo ""
...
OUTPUT_DIR="./work_dirs/sa2va_rl_6gpu_full_epoch"
LOG_FILE="/tmp/sa2va_rl_6gpu_full_epoch.log"

# TRAINING PARAMETERS
# With 6 GPUs, we can use slightly larger total batch size
PER_GPU_BATCH_SIZE=1   # Batch size per GPU
NUM_GENERATIONS=2       # CRITICAL: GRPO requires >=2 for std calculation
GRADIENT_ACCUM_STEPS=2  # Accumulate gradients to simulate larger batch

# Calculate effective batch size
EFFECTIVE_BATCH_SIZE=$((6 * PER_GPU_BATCH_SIZE * GRADIENT_ACCUM_STEPS))

echo "Multi-GPU Configuration:"
echo "  GPUs: 0,1,2,3,4,5 (6 GPUs)"
...
# Use only GPUs 0-5 (GPU 6-7 are in use)
export CUDA_VISIBLE_DEVICES=0,1,2,3,4,5

# PyTorch CUDA memory management - OOM optimization
# max_split_size_mb:128 - small blocks for better memory management
# NOTE: expandable_segments not supported on this PyTorch version, removed
...
echo ""
echo "GPU Status:"
nvidia-smi --query-gpu=index,name,memory.used,memory.total --format=csv | grep -E "^[0-5],"
