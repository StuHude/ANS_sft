...
from projects.llava_sam2.datasets import DescribeAnythingReferringDataset
...
#######################################################################
pretrained_pth = '/data/xiaoyicheng/Sa2VA/pretrained/4B_checkpoint/sa2va_4b_iter152k_fp32.pth/pytorch_model.bin'
...
model = dict(
    ...
    #fast_pool=True,
    #fast_pool_size=4,         # → 4*4=16 个 fast tokens/帧
...
# describe anything referring dataset
DESCRIBE_ANYTHING_CACHE_ROOT = DATA_ROOT + 'describe_anything_cache/'

# 多子配置：先从相对小的集合起步，跑通再加大
DESCRIBE_CFGS = ['SAV']  # 可继续加上 'COCOStuff', 'LVIS', 'Mapillary', 'OpenImages', 'PACO'

describe_anything_referring_dataset = dict(
    type=DescribeAnythingReferringDataset,
    hf_dataset_name='nvidia/describe-anything-dataset',
    hf_dataset_config=['SAV'],
    dataset_repo_dir='/data/xyc/DAM_repo',
    local_data_root='/data/xyc/DAM_data',
    ...
    lazy=True,
    repeats=1,
    instruction_template="Please generate a detailed description for the given image region.",
    hf_split='train',
    fast_pair_sampling=True,
    precomputed_total_pairs=1458157,
)
...
train_dataset = dict(
    type=ConcatDataset, datasets=[
        ...
        # describe anything referring
        describe_anything_referring_dataset, # 添加新数据集
    ]
)
...
#load_from = "/data/xiaoyicheng/Sa2VA/work_dirs/sa2va_4b/iter_155000.pth"
load_from = "/data/xiaoyicheng/Sa2VA/work_dirs/sa2va_4b_fresh_start/iter_6000.pth"


# whether to resume training from the loaded checkpoint
resume = True
work_dir = "/data/xiaoyicheng/Sa2VA/work_dirs/sa2va_4b_fresh_start"
